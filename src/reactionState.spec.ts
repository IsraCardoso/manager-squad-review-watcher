...
test("approved -> heavy_check_mark + approved message", () => {
  ...
  expect(replyMessage("approved", USER)).toBe(`<@${USER}> Revisado e aprovado ✔️`);
});
...
test("approved_with_comment -> heavy_check_mark + comment message", () => {
  ...
  expect(replyMessage("approved_with_comment", USER)).toContain("não bloqueia o merge");
});

test("changes_requested -> speech_balloon + changes message", () => {
  expect(terminalReaction("changes_requested")).toBe("speech_balloon");
  expect(replyMessage("changes_requested", USER)).toContain("Me marca aqui de novo quando ajustar");
});
